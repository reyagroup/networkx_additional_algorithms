...
void printTree(string s,multimap<double,treeNode,greater<double> >::iterator it_tM,ostringstream *oss,bool flip);
...
int main(int argc,char *argv[]){
  ...
  if( argc !=3 ){
    cerr << "Call: ./infomap <seed> <# attempts>" << endl;
    exit(-1);
  }
  
  int Ntrials = atoi(argv[2]);  // Set number of partition attempts
  string line;
  string buf;
  ...
  istringstream ss;
  while(Nnode == 0){ 
    if(getline(cin,line) == NULL){
      cerr << "the network file is not in Pajek format...exiting code1" << endl;
      exit(-1);
    }
    else{
      ss.clear();
      ...
      if(buf == "*Vertices" || buf == "*vertices" || buf == "*VERTICES"){
        ...
        cerr << "the network file is not in Pajek format...exiting code 2" << endl;
        exit(-1);
      }
    }
  }
  ...
  if(buf != "*Edges" && buf != "*edges" && buf != "*Arcs" && buf != "*arcs"){
    cerr << endl << "Number of nodes not matching, exiting" << endl;
    exit(-1);
  }
  ...
  repeated_partition(R,&node,greedy,true,Ntrials);
  ...
  for(int i=0;i<greedy->Nnode;i++){
    ...
    for(int j=0;j<Nmembers;j++){
      it_tM->second.members.insert(make_pair(size[node[i]->members[j]],nodeNames[node[i]->members[j]]));
    }
  }
...
/*
  ...
  k = 0;
  for(multimap<double,treeNode,greater<double> >::iterator it = treeMap.begin(); it != treeMap.end(); it++){
    outfile << k+1 << " \"" << it->second.members.begin()->second << "\" " << it->first << " " << node[k]->exit << endl;
    k++;
  }
  outfile << "*Nodes " << Nnode << endl;
  k = 1;
...
void printTree(string s,multimap<double,treeNode,greater<double> >::iterator it_tM,ostringstream *outfile,bool flip){
  ...
  if(it_tM->second.nextLevel.size() > 0){
    ...
    int i = 1;
    for(multimap<double,string,greater<double> >::iterator mem = it_tM->second.members.begin(); mem != it_tM->second.members.end(); mem++){
      if(flip){
        string cpy_s(s + to_string(i) + "\t\"" + mem->second + "\"" + to_string(mem->first));
        (*outfile) << cpy_s << endl;
      }
      else{
        string cpy_s(s + to_string(i) + "\t" + to_string(mem->first) + "\t\"" + mem->second + "\"");
        (*outfile) << cpy_s << endl;
      }
      i++;
    }  
